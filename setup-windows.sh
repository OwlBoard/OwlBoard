# OwlBoard - Setup Script for Windows (Git Bash)
# Este script es idéntico a setup.sh pero optimizado para Windows
...
echo -e "${CYAN}"
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║                 OwlBoard Setup Script (Windows)               ║"
echo "║              Configuración Automatizada Completa              ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

# Step 1: Verificar prerequisitos
echo -e "${BLUE}[1/6] Verificando prerequisitos...${NC}"

if ! command -v docker &> /dev/null; then
    echo -e "${RED}❌ Docker no está instalado. Por favor instala Docker Desktop.${NC}"
    echo -e "${YELLOW}   Descarga: https://www.docker.com/products/docker-desktop${NC}"
    exit 1
fi
...
if ! command -v openssl &> /dev/null; then
    echo -e "${RED}❌ OpenSSL no está disponible (debería venir con Git Bash).${NC}"
    exit 1
fi
...
# Step 1.5: Inicializar submódulos
...
if [ ! -f ".gitmodules" ]; then
    ...
    for module in "${critical_modules[@]}"; do
        if [ ! -d "$module" ] || [ -z "$(ls -A "$module" 2>/dev/null)" ]; then
            echo -e "${RED}❌ Submódulo '$module' no está clonado${NC}"
            all_present=false
        else
    ...
    if [ "$all_present" = false ]; then
        echo -e "${RED}❌ Algunos submódulos no están disponibles.${NC}"
        exit 1
    fi
fi
echo ""

# Step 2: Generar certificados
...
# Step 4: Limpiar instalación previa
echo -e "${BLUE}[4/6] Limpiando instalación previa...${NC}"
...
# Step 5: Construir e iniciar servicios
...
echo "  docker compose restart user_service # Reiniciar servicio"
